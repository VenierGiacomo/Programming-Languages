...
fun intersect (v1,v2) =
    case (v1,v2) of
     ...
     | (Point (x,y), Line (m,q)) => if real_close(y, m * x + q)
				     then v1
				     else NoPoints
     ...
     | (Line (m1,q1), Line (m2,q2)) =>
	if real_close(m1,m2) 
	then (if real_close(q1,q2)
	      then v1 (* same line *)
      ...
      | (Line (m1,q1), VerticalLine x2) => Point(x2, m1 * x2 + q1)
	...
	(case intersect(two_points_to_line seg, v2) of
	  ...
	  | LineSegment seg2 => 
		...
		then (* the segments are on a vertical line *)
		    (* let segment a start at or below start of segment q *)
		    let 
			...
			then LineSegment(bXstart,bYstart,bXend,bYend) (* q inside a *)
			else LineSegment(bXstart,bYstart,aXend,aYend) (* overlapping *)
		    end
		else (* the segments are on a (non-vertical) line *)
		    (* let segment a start at or to the left of start of segment q *)
		    let 
			val ((aXstart,aYstart,aXend,aYend),
